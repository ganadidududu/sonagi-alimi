...
// ---------- Cute face weather icons (ported from the app's WeatherIconView) ----------

type WeatherKind = "sun" | "cloud" | "shower" | "umbrella" | "drops";

function WeatherGlyph({ kind }: { kind: WeatherKind }) {
  const SUN = "#FFCF5A";
  const CLOUD = "#FFFFFF";
  const DROP = "#7FB4EC";
  const sunFace = "rgba(120,80,10,.8)";
  const cloudFace = "rgba(70,95,130,.78)";
  ...
  const sunFaceEl = (cx: number, cy: number, r: number) => (
    <g>
      {rays(cx, cy, r, r * 0.5, r * 0.3)}
      <circle cx={cx} cy={cy} r={r} fill={SUN} />
      <circle cx={cx - r * 0.28} cy={cy} r={1.9} fill={sunFace} />
      <circle cx={cx + r * 0.28} cy={cy} r={1.9} fill={sunFace} />
      <path d={`M${cx - r * 0.25} ${cy + r * 0.28} q${r * 0.25} ${r * 0.24} ${r * 0.5} 0`} stroke={sunFace} strokeWidth={1.8} fill="none" strokeLinecap="round" />
    </g>
  );

  const cloudEl = (withFace: boolean) => (
    <g>
      <circle cx={23} cy={34} r={12} fill={CLOUD} />
      <circle cx={43} cy={32} r={13} fill={CLOUD} />
      <circle cx={33} cy={25} r={12.5} fill={CLOUD} />
      <rect x={12} y={32} width={40} height={15} rx={7.5} fill={CLOUD} />
      {withFace && (
        <g>
          <circle cx={27} cy={34} r={1.9} fill={cloudFace} />
          <circle cx={39} cy={34} r={1.9} fill={cloudFace} />
          <path d="M29 39 q4 3.6 8 0" stroke={cloudFace} strokeWidth={1.8} fill="none" strokeLinecap="round" />
        </g>
      )}
    </g>
  );

  const drops = (pts: [number, number][]) => (
    <g>
      {pts.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r={3} fill={DROP} />
      ))}
    </g>
  );

  let content: React.ReactNode = null;
  if (kind === "sun") content = sunFaceEl(32, 32, 14);
  else if (kind === "cloud") content = cloudEl(true);
  else if (kind === "drops") content = drops([[20, 26], [32, 32], [44, 26], [32, 40]]);
  else if (kind === "shower")
    content = (
      <g>
        <g transform="translate(8 6) scale(0.55)">{sunFaceEl(14, 14, 12)}</g>
        {cloudEl(true)}
        {drops([[24, 52], [33, 55], [42, 52]])}
      </g>
    );
  else if (kind === "umbrella")
    content = (
      <g>
        <path d="M32 12 C18 12 8 22 6 33 L58 33 C56 22 46 12 32 12 Z" fill={DROP} />
        <path d="M6 33 Q13 27 20 33 Q26 27 32 33 Q38 27 44 33 Q51 27 58 33" fill="none" stroke="rgba(47,74,109,.35)" strokeWidth={1.3} />
        <rect x={30.6} y={33} width={2.8} height={18} rx={1.4} fill="#CDD8E6" />
        <path d="M31 51 q0 5 6 4.5" fill="none" stroke="#CDD8E6" strokeWidth={2.6} strokeLinecap="round" />
        <circle cx={32} cy={11} r={2} fill="#CDD8E6" />
      </g>
    );

  return (
    <svg viewBox="0 0 64 64" width="100%" height="100%" style={{ display: "block", overflow: "visible", filter: "drop-shadow(0 3px 8px rgba(40,60,95,.18))" }}>
      {content}
    </svg>
  );
}

function WeatherDeco({ kind, x, y, size, rotate }: { kind: WeatherKind; x: number; y: number; size: number; rotate: number }) {
  return (
    <div
      style={{
        position: "absolute",
...
function SlideBackground({
  ...
  // Scatter the app's cute weather glyphs in non-critical corners, varied per
  // slide so the deck doesn't repeat. They sit behind the phone/headline, so
  // they peek out around the device rather than competing with it.
  const decoSets: { kind: WeatherKind; x: number; y: number; size: number; rotate: number }[][] = [
    [
      { kind: "sun", x: 4, y: 5, size: 17, rotate: -8 },
      { kind: "cloud", x: 78, y: 12, size: 20, rotate: 6 },
      { kind: "umbrella", x: 82, y: 74, size: 15, rotate: 14 },
    ],
    [
      { kind: "cloud", x: 3, y: 8, size: 20, rotate: -6 },
      { kind: "drops", x: 84, y: 16, size: 12, rotate: 0 },
      { kind: "shower", x: 79, y: 76, size: 17, rotate: 8 },
    ],
    [
      { kind: "umbrella", x: 5, y: 9, size: 16, rotate: -12 },
      { kind: "sun", x: 80, y: 10, size: 16, rotate: 10 },
      { kind: "cloud", x: 6, y: 78, size: 18, rotate: -5 },
    ],
    [
      { kind: "sun", x: 82, y: 8, size: 17, rotate: 9 },
      { kind: "cloud", x: 3, y: 14, size: 18, rotate: -7 },
      { kind: "drops", x: 84, y: 76, size: 12, rotate: 6 },
    ],
    [
      { kind: "shower", x: 4, y: 7, size: 18, rotate: -9 },
      { kind: "umbrella", x: 82, y: 13, size: 15, rotate: 12 },
      { kind: "cloud", x: 80, y: 76, size: 18, rotate: 5 },
    ],
  ];
  const seed = slide.id.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
  const deco = decoSets[seed % decoSets.length];
  return (
    <div
      style={{
        position: "absolute",
    ...
    >
      <Blob cW={cW} color={theme.accent} x={-15} y={-10} size={55} opacity={inverted ? 0.25 : 0.28} />
      <Blob cW={cW} color={theme.accent} x={70} y={75} size={45} opacity={inverted ? 0.18 : 0.22} />
      {deco.map((d, i) => (
        <WeatherDeco key={i} {...d} />
      ))}
    </div>
